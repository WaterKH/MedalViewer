...
public class MedalAbilityParser
{
    ...
    private static readonly Regex FillAndCureRegex = new Regex(@"^Fills (\d+) gauges(?: and (cures))?");
    ...
    private void BoostsSapsAssignment(GroupCollection results, MedalAbility ability, string lowersRaises, string turnsAttacks, string tierValue = "")
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            switch (strDef)
            {
                case "strength":
                case "STR":
                    foreach (var s in medalCombatAbilities)
                        ability.STR.Add(s);
                    break;
                case "defense":
                case "DEF":
    ...
    private Regex updatedBoostsSapsRegex =
        new Regex(
            @"(\w*?)(?:-?)\s?(strength|STR|defense|DEF)?\s?(?:&|and)?\s?(\w*)(?:-?\w*)\s?(strength|STR|defense|DEF)");
    ...
    private void ParseUpdatedRaiseLower(GroupCollection resultGroups, MedalAbility ability)
    {
        ...
        var turns = resultGroups[1].Value;
        var persistantAmount = 0;
        var direction = "";
